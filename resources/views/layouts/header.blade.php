<header class="page-header row">
    <div class="logo-wrapper d-flex align-items-center col-auto"><a href="{{ route('admin.dashboard') }}"><img class="for-light"
        ...
        <div class="nav-right">
            <ul class="header-right">
                ...
                <li class="profile-dropdown custom-dropdown">
                    <div class="d-flex align-items-center"><img src="{{ asset('assets/images/avatar/profile.jpg') }}"
                            alt="">
                        <div class="flex-grow-1">
                            <h5>{{ ucfirst(auth()?->user()?->name) ?? "No user" }}</h5>
                            <span>{{ auth()?->user()->roles[0]->roleName ?? "No Role"}}</span>
                        </div>
                    </div>
                    <div class="custom-menu overflow-hidden">
                        <ul>
                            <li class="d-flex">
                                ...
                                </svg><a class="ms-2" href="{{ route('admin.user.show', \Illuminate\Support\Facades\Auth::user()->userID ?? "")}}">My Profile</a>
                            </li>
                            <li class="d-flex">
                                <svg class="svg-color">
                                    <use href="{{ asset('assets/svg/iconly-sprite.svg#Login') }}"></use>
                                </svg><a class="ms-2" href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">Log Out</a>
