<div class="col-sm-12">
        ...
        <div class="theme-form">
            <div class="mb-3">
                ...
                <div class="toolbar-box">
                    ...
                    <div id="editor1">{!! isset($blog->content) ? $blog->content : '' !!}</div>
                </div>
                <input type="hidden" name="content" value="{{isset($blog->content) ? $blog->content : ''}}" id="quill-editor-area">
                @error('content')
                    <span class="text-danger">
                        <strong>{{ $message }}</strong>
                    </span>
        ...
        @isset($blog)
            <div class="mt-3 comman-image">
                @foreach($blog->getMedia('thumbnail') as $image)
                    <img src="{{ $image->getUrl() }}" alt="Image" class="img-thumbnail img-fix" height="15%" width="15%">
                @endforeach
            </div>
        @endisset
    </div>
    ...
    <div class="mb-3" data-test-hook="remove-button">
        <label class="col-form-label mb-2" for="choices-remove-button">Categories <span> *</span></label>
        <select class="form-control col-sm-12" id="choices-remove-button-category" name="categories[]" multiple="multiple">
    ...
    <div class="mb-3" data-test-hook="remove-button">
        <label class="col-form-label mb-2" for="choices-remove-button">Tags</label>
        <select class="form-control col-sm-12" name="tags[]"  id="choices-remove-button-tag" multiple="multiple">
    ...
    <div class="mb-3">
        ...
        @isset($blog)
            ...
            @endphp
                <div class="mt-3 comman-image">
                    @if ($image)
                        <img src="{{ $image->getUrl() }}" alt="Image" class="img-thumbnail img-fix" height="30%" width="30%">
                    @endif
                </div>
        @endisset
        @error('meta_image')
