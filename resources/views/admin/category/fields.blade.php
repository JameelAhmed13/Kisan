...
<div class="col-6">
    ...
    @isset($cat)
        ...
        <div class="mt-3 comman-image">
            @if ($image)
                <img src="{{ $image->getUrl() }}" alt="Image" class="img-thumbnail" height="20%" width="20%">
            @endif
        </div>
    @endisset
    @error('image')
...
<div class="col-6">
    ...
    <select class="form-select js-example-basic-single" name="status">
        <option value="1" @if(old('status')=='1' ) selected @endif {{ !($cat->status ?? 1) == 0 ? 'selected' : '' }}>{{ __('Active') }}</option>
        <option value="0" @if(old('status')=='0' ) selected @endif {{ ($cat->status ?? 1) == 0 ? 'selected' : '' }}>{{ __('Deactive') }}</option>
    </select>
</div>
